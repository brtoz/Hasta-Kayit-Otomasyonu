...
{
    public partial class MuayeneGörüntüle : Form
    {
        OleDbConnection connection;
        string connectionString = @"provider=microsoft.jet.oledb.4.0; data source = HastaKayit.mdb";

        public MuayeneGörüntüle()
        {
            InitializeComponent();
            connection = new OleDbConnection(connectionString);
        }

        private void MuayeneGörüntüle_Load(object sender, EventArgs e)
        {
            string connectionString = @"provider=microsoft.jet.oledb.4.0; data source = HastaKayit.mdb";
            string query = "SELECT Hasta_Id, Klinik, Doktor FROM muayene";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    connection.Open();
                    string deleteQuery = "DELETE FROM muayene WHERE Hasta_Id = @PatientID";
